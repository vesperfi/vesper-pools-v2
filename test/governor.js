...
contract('GovernorAlpha', async function (accounts) {
  ...
  async function createProposal() {
    ...
    await vvsp.delegate(proposer, {from: proposer})
  ...
  beforeEach(async function () {
    ...
    governor = await GovernorAlpha.new(timelock.address, vvsp.address, owner)
    ...
    await vsp.approve(vvsp.address, oneMillion, {from: owner})
    await vvsp.deposit(oneMillion, {from: owner})
    const vvspBalance = await vvsp.balanceOf(owner)
    // Get 2% vVSP to propose actions, also wait for withdraw lock
    await time.increase(25 * 60 * 60)
    await vvsp.transfer(proposer, vvspBalance.div(new BN(50)))
  })
  ...
  describe('Queue proposal', function () {
    ...
    beforeEach(async function () {
      await vvsp.delegate(proposer, {from: owner})
      proposalId = await createProposal()
      await time.advanceBlock()
      await time.advanceBlock()
      await governor.castVote(proposalId, true, {from: proposer})
